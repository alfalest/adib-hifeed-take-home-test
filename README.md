...
1. **🧠 Backend API (Pusat Data / Otak Sistem)**
   ...
   - **Fitur Keamanan (_Atomic Transaction_):** Mencegah kesalahan hitung jika ada 2 petugas yang melakukan scan bersamaan. Stok tidak akan pernah bernilai minus.
...
3. **📱 Mobile Scanner (Aplikasi HP untuk Staf Lapangan)**
   ...
   - **Fitur:**
     ...
     - **Konfirmasi Masuk/Keluar:** Masukkan jumlah pakan yang diterima (_Inbound_) atau dipakai (_Dispatch_).
...
_(Jika belum ada, download dan install di: [https://nodejs.org](https://nodejs.org))_
...
| ------ | ---------------- | ------------------------------------------------------- |
| `GET`  | `/items`         | Mengambil daftar master pakan & status _Low Stock_      |
